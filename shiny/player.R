...
Player <- R6Class("Player",
                  public = list(
                    ...
                    nobuy = NA,      #N期後降低買股比率
                    # 初始化變項
                    initialize = function(id,cash,stock,type,setting,anchoring,anchortrial,total,herdb,herds,inversiveb,inversives,downratio,nobuy){
                      # 初始值設定
                    ...
                    decide = function(Market){
                      ...
                      }  else if(self$type == "Best"){
                        if(Market$trial <= 45){
                          Prob = rmultinom(1, size = 1, prob = c(1, 0, 0))
                        } else {
                          Prob = rmultinom(1, size = 1, prob = c(0, 0, 1))
                        }
                      }  else {
                        fairh = 1
                        fairl = 1
